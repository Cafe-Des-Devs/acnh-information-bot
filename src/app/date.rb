...
module AcnhBot
  ...
  class Date
    ...
    def format(format_string, language = :fr)
      ...
      return CONSOLE_LOGGER.warn("No method found for #{language} in #{File.path(Pathname.new("./src/date_formats/#{language}.rb"))}") unless AcnhBot::Locales.respond_to?(language.to_s)

      language_attributes = AcnhBot::Locales.method(language.to_s).call
      matches = {
        :YYYY => @time.year,
